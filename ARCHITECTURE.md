...
require it): (a) enable Pages on `vitals` branch if free, (b) the hub renders
every member repo's dashboard (primary path — always works), (c) third-party
raw-HTML preview services exist (raw.githack.com, htmlpreview.github.io) but
are best-effort; don't document them as the official path.
...
auth for public repos), builds an aggregate dashboard on GitHub Pages and a
combined `REPORT.md` (this is the grant-report artifact). The hub also runs a
**watchdog**: flags repos whose `VITALS.json` is > 3 days stale (usually the
disabled-cron problem, §7).
